...
static void *get_aim(void *player) {
    return *(void **)((uint64_t) player + 0x00);
}
    ...
    if (player != NULL) {
        ...
        if (myPlayer) {
            ...
            if (aim && enemyPlayer && !isPlayerDead(enemyPlayer)) {
                void* aimingdata = *(void **)((uint64_t) get_aim(myPlayer) + 0x00000);
                if (aimingdata) {
                    ...
                    if (angle.x <= -275.0f)
                        angle.x += 360.0f;
                    *(Vector3 *)((uint64_t)aimingdata + 0x0000) = angle;
                    *(Vector3 *)((uint64_t)aimingdata + 0x0000) = angle;
                }
            }
        }
...
MSHookFunction((void *) getAbsoluteAddress(targetLibName, 0x000000), (void *) playerupdate, (void **) &old_playerupdate);
IsLocal = (bool (*)(void *)) getAbsoluteAddress(targetLibName, 0x0000000);
GetPlayerHealth = (int (*)(void *)) getAbsoluteAddress(targetLibName, 0x000000);
GetPlayerTeam = (int (*)(void *)) getAbsoluteAddress(targetLibName, 0x00000000);
